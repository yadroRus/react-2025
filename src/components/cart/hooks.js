...
export const useCart = () => {
  ...
  useEffect(() => {
    const onBodyClick = (e) => {
      // todo NOTE: выявленна ошибка, при клике по кнопке "убрать", которая внутри корзины (.js-cart-area), происходит закрытие корзины, т.е. эта кнопка как-бы вне общего контейнера воспринимается, хотя она внути него, почему так - хз, в обычном JS такой проблемы бы не возникло
      /**
